...
class Hourly: public Employee{
	...
	void swap(Employee **EmployeeList, int i, int j){
...
int lower, upper, sortflag, sml, scan;
    ...
    while((lower<upper)&&(sortflag==1)){           
        ...
        sortflag = 0;                               
        while(scan<n-1){                   
            if(EmployeeList[scan]->NetPay > EmployeeList[scan+1]->NetPay){
				swap(EmployeeList,scan,scan+1);
                sortflag=1;
                if(EmployeeList[scan]->NetPay < EmployeeList[sml]->NetPay) sml=scan;
            }
        scan++;
        }
    swap(EmployeeList, lower, sml);        
    }
}
